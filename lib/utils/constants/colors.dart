import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';

class AppColors {
  static const Color primary = Color(0xFFE2EBED);
  static const Color secondary = Color(0xff222831);
  static const Color blue = Color(0xff205781);
  static const Color red = CupertinoColors.systemRed;
  static const Color tertiary = Color(0xFF3F463E);
  static const Color quarternary = Color(0xffEEEEEE);
  static const Color quinary = Colors.white;
  static const Color dark = Color(0xFF000000);
  static const Color grey = Color(0xFFDDDDDD);
  static const Color darkGrey = Color(0xff5b5c5e);
  static const Color green = Color(0xff20ae3a);

  static const Color prettyDark = Color(0xff181C14);
  static const Color prettyBlue = Color(0xff0097b2);
  static const Color prettyGrey = Color(0xffd1cece);

  static const Color darkBackground = Color(0xff1E1F22);
  static const Color lightDarkBackground = Color(0xff2B2D30);
}
