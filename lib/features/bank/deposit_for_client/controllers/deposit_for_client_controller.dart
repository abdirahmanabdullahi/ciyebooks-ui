...
import '../Deposit_for_client_model.dart';

class DepositForClientController extends GetxController {
  static DepositForClientController get instance => Get.find();
  ...
  final depositedAmount = 0.0.obs;
  final isButtonEnabled = false.obs;
  final isLoading = false.obs;

  RxList<AccountModel> accounts = <AccountModel>[].obs;
  final currency = [].obs;
  final depositForClientFormKey = GlobalKey<FormState>();
  ...
  void updateButtonStatus() {
    isButtonEnabled.value = from.text.isNotEmpty && accountNo.text.isNotEmpty && amount.text.isNotEmpty && depositedCurrency.text.isNotEmpty && (num.parse(amount.text) > 0);
  }
                              ...
                              children: [
                                ...
                                pw.Row(
                                  children: [
                                    ...
                                    pw.Text(depositedCurrency.text.trim(), style: pw.TextStyle(color: PdfColors.black, fontWeight: pw.FontWeight.bold, font: ttf)),
                                  ],
                                ),
                                pw.SizedBox(
                                  height: 10,
  ...
  Future createPayment(BuildContext context) async {
    ...
    try {
      ...
      depositedAmount.value = double.tryParse(amount.text.trim()) ?? 0.0;

      if (depositedAmount.value > cashBalance.value) {
      ...
      if (depositedAmount == cashBalance) {
        Get.snackbar(
          icon: Icon(
            Icons.cloud_done,
            color: Colors.white,
          ...
          shouldIconPulse: true,
          "Zero cash warning",
          'If you make this deposit, you will have to cash left',
      ...
      ///Doc references
      final paymentRef = db.collection('Users').doc(_uid).collection('transactions').doc('DPST-${counters['bankDepositCounter']}');
      ...
      final newPayment = DepositForClientModel(
          transactionId: 'DPST-${counters['bankDepositCounter']}',
          transactionType: 'payment',
          accountFrom: from.text.trim(),
          currency: depositedCurrency.text.trim(),
          amountDeposited: double.tryParse(amount.text.trim()) ?? 0.0,
          dateCreated: DateTime.now(),
          description: description.text.trim());

      ///Update account
      batch.update(accountRef, {'Currencies.${depositedCurrency.text.trim()}': FieldValue.increment(-num.parse(amount.text.trim()))});
