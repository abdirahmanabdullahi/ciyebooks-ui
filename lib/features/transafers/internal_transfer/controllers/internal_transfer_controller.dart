...
class InternalTransferController extends GetxController {
  static InternalTransferController get instance => Get.find();
  ...
  onInit() {
    ...
    receivingAccountName.addListener(updateButtonStatus);
  ...
  void updateButtonStatus() {
    ...
    isButtonEnabled.value =transferredCurrency.text.isNotEmpty
    &&transferringAccountNo.text.isNotEmpty
        &&receivingAccountNo.text.isNotEmpty
    &&
        transferringAccountNo.text !=
            receivingAccountNo.text &&
        transferredAmount.text.isNotEmpty &&

        (num.parse(transferredAmount.text) > 0);
  }
  ...
  fetchTotals() async {
    ...
    if (documentSnapshot.exists && documentSnapshot.data() != null) {
      ...
      transactionCounter.value = counters['internalTransferCounter'];
    }
  }
                              ...
                              children: [
                                ...
                                pw.Row(
                                  children: [
                                    ...
                                    pw.Text('INTRNSFR-${counters['internalTransferCounter']}', style: pw.TextStyle(color: PdfColors.black, fontWeight: pw.FontWeight.bold, font: ttf)),
                                  ],
                                ),
                                pw.SizedBox(
                                  height: 10,
                                ...
                                pw.Row(
                                  children: [
                                    ...
                                    pw.Text(receivingAccountName.text.trim(), style: pw.TextStyle(color: PdfColors.black, fontWeight: pw.FontWeight.bold, font: ttf)),
                                  ],
                                ),
                                pw.SizedBox(
                                  height: 10,
...
print(isButtonEnabled.value);
    print('[[[[[[[[[[[[[[[[[[[[[[object]]]]]]]]]]]]]]]]]]]]]]');
   print(transferringAccountNo.text);
   print(receivingAccountNo.text);
   print(transferredAmount.text);
    ...
    try {
      ...
      ///Doc references
      final internalTransferRef = db.collection('Users').doc(_uid).collection('transactions').doc('INTRNSFR-${counters['internalTransferCounter']}');
      ...
      final receivingAccountRef = db.collection('Users').doc(_uid).collection('accounts').doc('PA-${receivingAccountNo.text.trim()}');

      ///Get data from the controllers
      final newInternalTransfer = InternalTransferModel(
          transactionId: 'INTRNSFR-${counters['internalTransferCounter']}',
      ...
      batch.update(receivingAccountRef, {'Currencies.${transferredCurrency.text.trim()}': FieldValue.increment(num.parse(transferredAmount.text.trim()))});

      ///Create payment transaction
      batch.set(internalTransferRef, newInternalTransfer.toJson());

      ///update cash balance
      batch.update(cashRef, {"cashBalances.${transferredCurrency.text.trim()}": FieldValue.increment(-num.parse(transferredAmount.text.trim()))});
      ...
      batch.update(counterRef, {"transactionCounters.internalTransferCounter": FieldValue.increment(1)});
